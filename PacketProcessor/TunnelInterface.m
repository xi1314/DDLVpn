...
#define kTunnelInterfaceErrorDomain [[[NSBundle mainBundle]infoDictionary] objectForKey:@"CFBundleIdentifier"]
...
+ (NSError *)setupWithPacketTunnelFlow:(NEPacketTunnelFlow *)packetFlow {
    NSLog(@"setupWithPacketTunnelFlow--%@",packetFlow);
    if (packetFlow == nil) {
    ...
    if (error) {
        NSLog(@"Unable to pipe.%@",[NSString stringWithFormat:@"UDP bind fail(%@).", [error localizedDescription]]);
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext {
    ...
    NSLog(@"src_ip%d --- src_port %d",src_ip,src_port);
